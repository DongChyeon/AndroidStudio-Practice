...
import com.example.dongstagram.LoginActivity
import com.example.dongstagram.MainActivity
...
import com.example.dongstagram.navigation.model.FollowDTO
    ...
    fun requestFollow() {
        ...
        firestore?.runTransaction { transaction ->
            ...
            if (followDTO.followings.containsKey(uid)) {
                // 다른 사람이 나를 팔로우하고 있을 때 팔로우를 없애줌
                followDTO?.followingCount = followDTO?.followingCount - 1
                followDTO?.followings.remove(uid)
            } else {
                // 다른 사람이 나를 팔로우하고 있지 않을 때 팔로우를 추가해줌
                followDTO?.followingCount = followDTO?.followingCount + 1
                followDTO?.followings[uid!!] = true
            }
            transaction.set(tsDocFollowing, followDTO)
        ...
        firestore?.runTransaction { transaction ->
            ...
            if (followDTO!!.followers.containsKey(currentUserUid)) {
                // 이미 팔로우하고 있는 사람을 팔로우할 때 팔로워를 없애줌
                followDTO!!.followerCount = followDTO!!.followerCount - 1
                followDTO!!.followers?.remove(currentUserUid)
            } else {
                // 팔로우를 하고 있지 않는 사람을 팔로우할 때 팔로워를 추가해줌
